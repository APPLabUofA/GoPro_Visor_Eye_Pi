# -*- coding: utf-8 -*-
# Note, must clear variables between reruns
import numpy as np
import cv2
import mne
import pandas as pd
...
# %% # Are we saving an output file (file with overlaid filters/bounders/manipulations)?
 # # # # # # # # Was output at a different resolution to what was expecting
out_format = '.avi'
...
if exp_num == 1: # original exp (2018)
    ...
    start_flash = [0,25000,0,15000,12500]
    past_last = [0,294960,0,253950,973436] #294960

Vid_1_Dur = [0,0,0,1059.208,]
...
while in_frame == True: 
    ...
    if np.sum(diff) >= broad_thresh: # frame to frame subtraction
...
#            cv2.imshow("",last_countie)
        ...
        ######################################## Find Edges ######################
            if Trigger_State[count,1] != Trigger_State[count-1,1]: # only look in contoured frames that are not followed by another flash (as per the last Trigger_State)
                front_back = 1
                edge = []
                while front_back >= -1:
                    previous_frame = img1
        ...
        #                jump = 100^i
                        ...
                        ret, frame_current = cap.read() #389
        ...
        #                        broad_threshold += 20000
                            ######################
                            for i in range(3):
                                currenty_frame = current_frame-i + 1
                                cap.set(1, currenty_frame)
                                ret, frame_current = cap.read()
                                frame_current = frame_current[240:480,212:636,:]
                                print(currenty_frame)
        
                                cv2.putText(frame_current, str(i-1), (x, y+h), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), lineType=cv2.LINE_AA) 
...
df1['Frame'] = (df1['Frame']/240) # Change from the conversion
...
if anal ==1:
    ...
    df4 = df1.copy() # copy DataFrame 
#    df4 = df4.values # convert from Pandas DataFrame to a numpy structure
#    df4 = np.append(df4, np.zeros((trial_count-len(df1.index),3)), axis=1)
    df4 = df4.diff()
    misses = (df4[df4.Time > 3].index)
    
#    for x in range:
#        df2.loc[-1] = [np.nan, np.nan]  # adding a row
#        df2.index = df2.index + 1  # shifting index
#        df2 = df2.sort_index()  # sorting by index
    ...
    reg =  LinearRegression().fit(df4[:,1].reshape(-1,1), df4[:,5].reshape(-1,1)) # From the pi times we are predicting the eeg times
