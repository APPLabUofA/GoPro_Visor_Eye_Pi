...
# %% Manual Adjust for the sake of plotting difference between EEG 

# For participant 004
df3['Time'][250:-1] = df3['Time'][251::]
df3['Event'][250:-1] = df3['Event'][251::] 
df3 = df3.reset_index()
df3['Difference'] = df3['eeg_times'] - df3['Time']
...
export_csv = df3.to_csv (r'M:\Data\GoPro_Visor\Experiment_1\Video_Times\Dataframe_df3_whole_final_00' + str(par) + '.csv', index = None, header=True) #Don't forget to add '.csv' at the end of the path
# or load in
df3 = pd.read_csv((r'M:\Data\GoPro_Visor\Experiment_1\Video_Times\Dataframe_df3_whole_final_00' + str(par)) + '.csv', sep=',') 


# Latency plot
plt.close('all')
plt.figure(0)
plt.plot(df3['Time'], df3['index'], 'k--', label='Camera Times')
